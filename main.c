...
int main(int ac, char **av)
{
	...
	while (1)
	{
		...
		{
			prompt_bytes_written = write(1, prompt, _strlen(prompt));
		...
		while (command_token != NULL)
		{
			...
			command[i] = _strdup(command_token);
			if (command[i] == NULL)
				continue;
		...
		if (path == NULL)
		{
			...
			while (directories_token != NULL)
			{
				...
				directories[directories_i] = _strdup(directories_token);
				if (PATH == NULL)
					continue;
			...
			while (directories[searching_i] != NULL)
			{
				...
				while (entry != NULL)
				{
					if (_strcmp(command[0], entry->d_name) == 0)
					{
						found = 1;
						break;
					}
		...
		else if (child_pid == 0)
		{
			...
			{
				size = _strlen(directories[searching_i]) + _strlen(line) + 2;
				...
				_strcpy(concat_path, directories[searching_i]);
				_strcat(concat_path, "/");
				_strcat(concat_path, command[0]/*line*/);
